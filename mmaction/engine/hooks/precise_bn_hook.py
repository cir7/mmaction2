...
def update_bn_stats(model: nn.Module,
    ...
    Args:
        ...
        num_iters (int): The number of iterations to update the bn stats.
            Defaults to 200.
    ...
    # if the size of dataloader is less than num_iters,
    # use all the samples in dataloader.
    num_iters = min(num_iters, len(loader))
    ...
    if len(bn_layers) == 0:
        ...
        return
    print_log(
        f'{len(bn_layers)} BN found, run {num_iters} iters...', logger=logger)
    ...
    if rank == 0:
        prog_bar = ProgressBar(num_iters)

    for data in itertools.islice(loader, num_iters):
        data = model.data_preprocessor(data, False)
        model(**data)

        for i, bn in enumerate(bn_layers):
            running_means[i] += bn.running_mean / num_iters
            running_vars[i] += bn.running_var / num_iters
        if rank == 0:
            prog_bar.update()
...
class PreciseBNHook(Hook):
    ...
    def __init__(self, num_iters: int = 200, interval: int = 1) -> None:
        assert interval > 0 and num_iters > 0, "'interval' and " \
            "'num_iters' must be bigger than 0."
